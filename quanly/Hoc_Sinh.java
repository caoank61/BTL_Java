...
public class Hoc_Sinh {
	...
	public float Tinh_DiemTB(float Diem_Toan,float Diem_Ly,float Diem_Hoa,float Diem_Van,float Diem_Su,float Diem_Dia) {
		float tb = (Diem_Toan + Diem_Ly +Diem_Hoa + Diem_Van + Diem_Su + Diem_Dia)/6;
		return (float) Math.round(tb * 100) / 100;
	}
	public void Nhap_Thong_Tin_Hoc_Sinh () {
		...
		do {
			...
			Thang = sc.nextInt();
				if(KiemTra(Ngay, Thang, Nam) == false){
		...
		}while(KiemTra(Ngay, Thang, Nam)==false);
		do {
	...
	public void Xuat_Thong_Tin_Hoc_Sinh () {
		System.out.println("MSHS: " + MSHS +" || Ten: "+ HoTen +" || Lop: "+ Lop + " || Dia chi: "+ Dia_Chi + " || Ngay Thang Nam Sinh: "+Ngay +"/"+Thang+ "/"+Nam+" || Tuoi: "+Tinh_Tuoi()+" || Toan: "+Diem_Toan+" || Ly: "+Diem_Ly+" || Hoa: "+Diem_Hoa+" || Van: "+Diem_Van+" || Su: "+Diem_Su+" || Dia: "+Diem_Dia + " || Diem Trung Binh: " + Tinh_DiemTB(Diem_Toan,Diem_Ly,Diem_Hoa,Diem_Van, Diem_Su,Diem_Dia));
	}
	public boolean KiemTra(int Ngay, int Thang, int Nam){
		    //kiem tra ngay thang
